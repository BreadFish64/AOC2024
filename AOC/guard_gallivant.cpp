...
const std::array<Vec2D, 128> DIRECTION_VECTOR = [] {
    std::array<Vec2D, 128> init{};
    init[UP]    = {-1, 0};
    init[LEFT]  = {0, -1};
    init[DOWN]  = {1, 0};
    init[RIGHT] = {0, 1};
...
size_t PatrolCoverage(const mdspan<uint8_t, dextents<int32_t, 2>> grid, Pos2D pos) {
    size_t coverage{0};
    uint8_t direction{1};
...
bool PatrolLoops(const mdspan<uint8_t, dextents<int32_t, 2>> grid, Pos2D pos) {
    uint8_t direction{1};
    while (pos[0] >= 0 && pos[0] < grid.extent(0) && pos[1] >= 0 && pos[1] < grid.extent(1)) {
        uint8_t& cell{grid[pos]};
...
size_t PossibleLoops(const mdarray<uint8_t, dextents<int32_t, 2>>& masterGrid,
                     const mdspan<uint8_t, dextents<int32_t, 2>>& coverageGrid, const Pos2D initialPos) {
...
void AocMain(std::string_view input) {
    ...
    const Pos2D initialPos{static_cast<int32_t>(caretPos / inputGrid.stride(0)),
                           static_cast<int32_t>(caretPos % inputGrid.stride(0))};
